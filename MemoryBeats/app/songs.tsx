...
export default function SongsScreen() {
  ...
  const route = useRoute<{ key: string; name: string; params: { birthYear: number; location: string; childhoodStart: number; childhoodEnd: number } }>();
  const { params } = route;
  const birthYear = Number(params?.birthYear ?? 0);
  const location = String(params?.location ?? '');
  const childhoodStart = Number(params?.childhoodStart ?? 0);
  const childhoodEnd = Number(params?.childhoodEnd ?? 0);
